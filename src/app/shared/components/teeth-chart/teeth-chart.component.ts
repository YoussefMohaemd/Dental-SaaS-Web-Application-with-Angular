...
export class TeethChartComponent {
  ...
  readonly hasServiceColors = computed(() => Object.keys(this.serviceTeeth()).length > 0);
  ...
  getRotation(num: number): number {
    ...
    const rotations: Record<number, number> = { 8: 8, 7: 5, 6: 2, 5: 0, 4: 0, 3: 0, 2: 0, 1: 0 };
  ...
  glowOf(num: number): string {
    if (this.isSelected(num)) return `0 0 0 2px ${this.serviceColorOf(num) ?? '#2563EB'}40`;
    if (this.hoveredTooth() === num && !this.readOnly()) return '0 0 0 2px #2563EB20';
    return 'none';
  }
