...
export const LUCIDE_INNERS: Record<string, string> = {
  ...
  'bar-chart-3': '__ALIAS__chart-column',
  'check-circle-2': '__ALIAS__circle-check',
  'alert-circle': '__ALIAS__circle-alert',
  'alert-triangle': '__ALIAS__triangle-alert',
  'edit': '__ALIAS__square-pen',
  'edit-2': '__ALIAS__pen',
  'trash-2': '__ALIAS__trash',
  'check-square': '__ALIAS__square-check-big',
  'more-horizontal': '__ALIAS__ellipsis',
  'unlock': '__ALIAS__lock-open',
  'refresh': '__ALIAS__refresh-cw',
};

const WRAP_OPEN = (size: number) =>
  `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">`;

export function lucideSvg(name: string, size = 16): string {
  let inner = LUCIDE_INNERS[name];
  if (!inner) return "";
  if (inner.startsWith("__ALIAS__")) {
    const target = inner.slice("__ALIAS__".length);
    inner = LUCIDE_INNERS[target] ?? "";
  }
  if (!inner) return "";
  return `${WRAP_OPEN(size)}${inner}</svg>`;
}

export function withIconSize(svg: string, size?: number): string {
