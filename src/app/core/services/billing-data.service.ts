...
export class BillingDataService {
  ...
  readonly pendingCount = computed(() => this._records().filter(r => r.status === 'Pending').length);
  readonly overdueCount = computed(() => this._records().filter(r => r.status === 'Overdue').length);
  readonly paidCount = computed(() => this._records().filter(r => r.status === 'Paid').length);
  ...
  loadRecords(): void {
    ...
    this.http.get<BillingRecord[]>(this.API_URL).pipe(
      catchError(err => {
