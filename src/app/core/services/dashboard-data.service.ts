...
export class DashboardDataService {
  ...
  readonly currentWeek = computed<VolumeWeek>(() => {
    ...
    return weeks.length > 0
      ...
      : DashboardDataService.FALLBACK_WEEKS[0];
  });
  ...
  readonly weekMax = computed<number>(() =>
    Math.max(
      ...this.currentDays().map((d) => Math.max(d.orders, d.completed)),
      1,
    ),
  );
  ...
  loadVolume(): void {
    ...
    this.http
      .get<{ weeks: VolumeWeek[] }>(this.API_URL)
      .pipe(
        catchError((err) => {
          console.warn(
        ...
        next: (payload) => {
          this._weeks.set(
            payload?.weeks?.length
              ? payload.weeks
              : DashboardDataService.FALLBACK_WEEKS,
          );
