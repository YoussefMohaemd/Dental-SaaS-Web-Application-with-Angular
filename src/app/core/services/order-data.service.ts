...
export class OrderDataService {
  ...
  readonly statusCounts = computed(() => {
    const counts: Record<OrderStatus, number> = {
      'New': 0, 'Review': 0, 'Design': 0, 'Production': 0,
      'Quality Check': 0, 'Ready': 0, 'Completed': 0, 'Cancelled': 0
    };
    this._orders().forEach(o => counts[o.status]++);
    return counts;
  });
  readonly priorityCounts = computed(() => {
    const counts: Record<Priority, number> = { 'Low': 0, 'Normal': 0, 'High': 0, 'Urgent': 0 };
    this._orders().forEach(o => counts[o.priority]++);
    return counts;
  });
  readonly urgentOrdersCount = computed(() => this._orders().filter(o => o.priority === 'Urgent').length);
  readonly completedTodayCount = computed(() => this._orders().filter(o => o.status === 'Completed').length);
  ...
  loadOrders(): void {
    ...
    this.http.get<Order[]>(this.API_URL).pipe(
      catchError(err => {
  ...
  createOrder(order: Omit<Order, 'id' | 'orderNumber' | 'receivedAt' | 'updatedAt'>): Order {
