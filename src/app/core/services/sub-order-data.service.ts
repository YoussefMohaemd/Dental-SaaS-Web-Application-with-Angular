...
function buildDetailFromSummary(
  ...
  const forms: SubOrderDetail["forms"] = Array.from(
    { length: row.formsTotal },
    (_, index) => ({
      id: `${row.id}-form-${index + 1}`,
      label:
        row.formsTotal === 1
          ? `${row.service} Form`
          : `${row.service} Form ${index + 1}`,
      required: true,
      status: index < row.formsComplete ? "complete" : "incomplete",
    }),
  );

  const scans: SubOrderDetail["scans"] = Array.from(
    { length: row.scansTotal },
    (_, index) => ({
      id: `${row.id}-scan-${index + 1}`,
      label:
        row.scansTotal === 1
          ? `${row.service} Scan`
          : `${row.service} Scan ${index + 1}`,
...
const FALLBACK_SUB_ORDERS: SubOrder[] = [
  {
    ...
    dueDate: "2024-03-15",
    notes:
      "Standard surgical guide for dual implant placement at sites 14, 15, 24, 25. Straumann BLT Ø4.1mm.",
  },
  {
    id: "so-2",
    orderId: "ord-1",
    ...
    dueDate: "2024-03-20",
    notes:
      "Full mouth rehabilitation. Occlusal vertical dimension (OVD) increase of 3mm confirmed by Dr. Kim. Mutually protected occlusion concept.",
  },
  {
    id: "so-3",
    orderId: "ord-1",
    ...
    dueDate: "2024-04-01",
    notes:
      "Posterior zirconia crowns with shade A2 and internal characterization. Confirm margin type with Dr. Kim before fabrication.",
  },
  {
    id: "so-4",
    orderId: "ord-1",
    ...
    dueDate: "2024-03-10",
    notes:
      "Full treatment plan document to be reviewed by lab and clinic team jointly before proceeding.",
  },
];
...
const SUB_ORDER_DETAILS: Record<string, SubOrderDetail> = {
  "so-1": {
    ...
    forms: [
      {
        id: "f1",
        label: "Implant System & Diameter",
        required: true,
        status: "complete",
      },
      {
        id: "f2",
        label: "Bone Quality Assessment",
        required: true,
        status: "complete",
      },
      {
        id: "f3",
        label: "Surgical Protocol Notes",
        required: true,
        status: "complete",
      },
    ],
    scans: [
      ...
      {
        id: "s3",
        label: "Supporting Reference Files",
        format: "PDF / JPG",
        status: "uploaded",
      },
    ],
    activity: [
      {
        time: "2 hours ago",
        user: "Jessica R.",
        text: "Surgical guide approved and sent to fabrication.",
      },
      {
        time: "1 day ago",
        user: "Tom K.",
        text: "CBCT scan uploaded and reviewed.",
      },
      {
        time: "3 days ago",
        user: "Jessica R.",
        text: "Sub-order created from parent order.",
      },
    ],
  },
  "so-2": {
    ...
    forms: [
      {
        id: "f1",
        label: "Occlusal Concept Form",
        required: true,
        status: "complete",
      },
      {
        id: "f2",
        label: "VDO Change Documentation",
        required: true,
        status: "complete",
      },
      {
        id: "f3",
        label: "Clinical Notes & Photos",
        required: true,
        status: "incomplete",
      },
    ],
    ...
    activity: [
      {
        time: "30 min ago",
        user: "Jessica R.",
        text: "Bite scan still pending from clinic. Sent reminder to Dr. Kim.",
      },
      {
        time: "2 days ago",
        user: "Tom K.",
        text: "Upper and lower arch scans uploaded successfully.",
      },
      {
        time: "4 days ago",
        user: "Jessica R.",
        text: "GFMR sub-order created.",
      },
    ],
  },
  "so-3": {
    ...
    forms: [
      {
        id: "f1",
        label: "Shade & Material Selection",
        required: true,
        status: "incomplete",
      },
      {
        id: "f2",
        label: "Margin & Occlusion Specs",
        required: true,
        status: "incomplete",
      },
    ],
    ...
    activity: [
      {
        time: "5 days ago",
        user: "Jessica R.",
        text: "Final Restoration sub-order created. Waiting for scans from clinic.",
      },
    ],
  },
  "so-4": {
    ...
    forms: [
      ...
      {
        id: "f2",
        label: "Proposed Treatment Outline",
        required: true,
        status: "incomplete",
      },
    ],
    ...
    activity: [
      {
        time: "1 week ago",
        user: "Jessica R.",
        text: "Treatment Plan sub-order created.",
      },
    ],
  },
};
...
export class SubOrderDataService {
  ...
  private readonly _details = signal<Record<string, SubOrderDetail>>(
    structuredClone(SUB_ORDER_DETAILS),
  );
        ...
        next: (rows) => {
          ...
          rows.forEach((row) => {
            if (!(row.id in nextDetails)) {
              nextDetails[row.id] =
                toDetailFromJson(row) ?? buildDetailFromSummary(row);
            }
          });
        ...
        activity: [
          {
            time: "just now",
            user: "System",
            text: `${row.service} sub-order created from order ${orderId}.`,
          },
        ],
      };
    }
  ...
  saveFormItem(
    ...
    const updatedForms = detail.forms.map((form) => {
      ...
      const nextStatus: SubOrderFormItemStatus =
  ...
  saveScanFiles(
    ...
    const updatedScans = detail.scans.map((scan) => {
      ...
      const nextStatus: SubOrderScanItemStatus =
        files.length > 0 ? "selected-local" : "missing";
      return {
        ...scan,
        localFiles: files,
        status: nextStatus,
  ...
  private refreshSubOrderCounts(subOrderIds: string[]): void {
    ...
    this._subOrders.update((rows) =>
      rows.map((row) => {
        ...
        const formsComplete = detail.forms.filter(
          (form) => form.status === "complete",
        ).length;
        const scansTotal = detail.scans.length;
        const scansComplete = detail.scans.filter(
          (scan) =>
            scan.status === "uploaded" || scan.status === "selected-local",
        ).length;
