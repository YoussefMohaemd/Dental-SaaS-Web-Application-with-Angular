...
describe('CreateOrderComponent', () => {
  ...
  it('should preserve service/form/tooth/file references on submit', () => {
    ...
    component.setField('clinicId', 'c1');
    component.toggleService('gfmr');
    component.setServiceDetail('gfmr', 'serviceNotes', 'Submit-order contract test');
    component.setServiceForm('gfmr', 'clinicalNotes', 'Clinical notes');
    component.activeServiceForTeeth.set('gfmr');
    component.toggleTooth(11);

    const orderService = (component as any).orderService as { createOrder: jasmine.Spy };
    const subOrderService = (component as any).subOrderService as { createForOrder: jasmine.Spy };
    ...
    expect(createdOrderArg.creationData.services[0].serviceDetails.serviceNotes).toBe('Submit-order contract test');
    expect(createdOrderArg.creationData.services[0].serviceForm.clinicalNotes).toBe('Clinical notes');
    expect(createdOrderArg.creationData.services[0].selectedTeeth).toEqual([11]);
    ...
    const createdRows = subOrderService.createForOrder.calls.mostRecent().args[1];
    expect(createdRows[0].creationData.serviceId).toBe('gfmr');
    expect(createdRows[0].creationData.selectedTeeth).toEqual([11]);
    expect((component as any).navigationService.navigate).toHaveBeenCalledWith('viewOrder', jasmine.any(Object));
  });
});
