...
describe('OrdersComponent', () => {
  ...
  it('should use human-readable labels in sort aria-labels', () => {
    ...
    expect(component.sortAriaLabel('orderNumber')).toBe('Sort by Order number, currently descending');
    expect(component.sortAriaLabel('status')).toBe('Sort by Status, currently unsorted');
  });
  ...
  it('should expose Lucide-equivalent SVG icons for actions', () => {
    for (const name of ['plus', 'download', 'refresh', 'lock', 'unlock', 'chevron-left', 'chevron-right']) {
  ...
  it('should navigate to view order and related records', () => {
    ...
    expect(nav.navigate).toHaveBeenCalledWith('viewOrder', { orderId: 'ord-1' });
    component.openPatient('pt1');
    expect(nav.navigate).toHaveBeenCalledWith('patientDetails', { patientId: 'pt1' });
    component.openDoctor('dr1');
    expect(nav.navigate).toHaveBeenCalledWith('doctorDetails', { doctorId: 'dr1' });
  });
