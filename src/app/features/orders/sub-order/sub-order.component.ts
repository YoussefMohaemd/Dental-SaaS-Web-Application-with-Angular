...
const EMPTY_DETAIL: SubOrderDetail = {
  id: "",
  forms: [],
  scans: [],
  activity: [],
...
export class SubOrderComponent {
  ...
  readonly subOrderId = computed(
    () => this.paramMap()?.get("subOrderId") ?? "",
  );
  ...
  readonly detail = computed(() => {
    return (
      this.subOrderService.getDetailByContext(
        this.orderId(),
        this.subOrderId(),
      ) ?? EMPTY_DETAIL
    );
  ...
  readonly scansUploaded = computed(
    () =>
      this.detail().scans.filter(
        (s) => s.status === "uploaded" || s.status === "selected-local",
      ).length,
  );
  ...
  readonly progress = computed(() => {
    const d = this.detail();
    const totalRequired =
      d.forms.filter((f) => f.required).length +
      d.scans.filter((s) => s.status !== "optional").length;
    const totalDone =
  ...
  onPostNote(): void {
    ...
    if (!sub) return;
    this.subOrderService.addActivityNote(
      sub.id,
  ...
  removeLocalFile(scanId: string, fileId: string): void {
    ...
    if (!scan) return;
    const remaining = (scan.localFiles ?? []).filter(
      (file) => file.id !== fileId,
    );
  ...
  scanStatusLabel(scan: SubOrderScanItem): string {
    if (scan.status === "selected-local")
      return "Selected locally (not uploaded to backend)";
  ...
  getIconSvg(name: string): string {
    const icons: Record<string, string> = {
      ...
      "scan-line":
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/><path d="M7 12h10"/></svg>',
      check:
      ...
      "chevron-right":
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>',
      "chevron-down":
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>',
      "alert-circle":
  ...
  private handleScanFiles(scanId: string, files: File[]): void {
    ...
    if (spaceLeft <= 0) {
      this.scanMessage.set(
        `Maximum ${this.maxFilesPerScan} files allowed for this requirement.`,
      );
      return;
    }
    ...
    const duplicateKeys = new Set(
      currentFiles.map((file) => `${file.name}::${file.sizeBytes}`),
    );
    let oversizeCount = 0;
    ...
    if (oversizeCount === 0 && duplicateCount === 0) {
      this.scanMessage.set(
        `${nextRows.length - currentFiles.length} file(s) selected locally for ${scan.label}.`,
      );
      return;
    }
