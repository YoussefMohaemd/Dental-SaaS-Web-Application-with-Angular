...
describe("SubOrderComponent", () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        provideRouter([]),
        {
          ...
          useValue: {
            paramMap: of(
              convertToParamMap({ orderId: "ord-1", subOrderId: "so-2" }),
            ),
  ...
  it("should render four tabs with forms/scans count badges", () => {
    ...
    const labels = tabs.map(
      (t) =>
        (t.nativeElement.textContent as string)
          .replace(/\s+/g, " ")
          .trim()
          .split(" ")[0],
    );
    expect(labels).toEqual(["overview", "forms", "scans", "activity"]);
    const badges = tabs.map((t) =>
      (t.nativeElement.textContent as string).replace(/\s+/g, " ").trim(),
    );
  ...
  it("should navigate to the resolved parent order", () => {
    ...
    component.goBack();
    expect(component["navigationService"].navigate).toHaveBeenCalledWith(
      "viewOrder",
      { orderId: "ord-1" },
    );
  });
  ...
  it("should return no detail for an unknown sub-order id", () => {
    expect(
      component["subOrderService"].getDetailByContext(
        "ord-1",
        "missing-sub-order",
      ),
    ).toBeUndefined();
  ...
  it("should return no detail for mismatched order context", () => {
    expect(
      component["subOrderService"].getDetailByContext("ord-9", "so-2"),
    ).toBeUndefined();
  });
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        provideRouter([]),
        {
          ...
          useValue: {
            paramMap: of(
              convertToParamMap({ orderId: "ord-33", subOrderId: "so-58" }),
            ),
            queryParamMap: of(convertToParamMap({})),
          },
        },
