...
const INITIAL_FILES: OrderFileEntry[] = [
  ...
  {
    id: "f5",
    name: "shade_guide_reference.jpg",
    type: "JPG",
    size: "1.6 MB",
    sizeBytes: 1677721,
    status: "uploaded",
...
export class OrderFilesComponent {
  ...
  addFiles(fileList: FileList): void {
    const existingNames = new Set(
      this.files().map((file) => file.name.toLowerCase()),
    );
    const rejected: string[] = [];
    const additions: OrderFileEntry[] = Array.from(fileList)
      .slice(0, 10)
      .flatMap((file, index) => {
  ...
  retryUpload(fileId: string): void {
    this.files.update((current) =>
      current.map((f) =>
        f.id === fileId
          ? { ...f, status: "uploading" as FileStatus, progress: 8 }
          : f,
      ),
    );
  ...
  fileIconLarge(fileType: string): string {
    return this.fileIcon(fileType).replace(
      'width="20" height="20"',
      'width="48" height="48"',
    );
  }
