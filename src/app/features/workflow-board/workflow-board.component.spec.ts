...
import {
  ...
  workflowStatusFromDropListId,
} from "./workflow-board.component";
...
describe("WorkflowBoardComponent", () => {
  ...
  it("should resolve workflow statuses from drop list ids", () => {
    expect(
      workflowStatusFromDropListId(component.columns, "workflow-col-ready"),
    ).toBe("Ready");
    expect(
      workflowStatusFromDropListId(component.columns, "missing-list"),
    ).toBeNull();
  });
});
