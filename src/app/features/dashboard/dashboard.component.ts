...
export class DashboardComponent implements OnInit {
  ...
  readonly activeCases = computed(
    () => this.cases().filter((c) => c.status !== "Closed").length,
  );
  readonly pendingReview = computed(
    () => this.orders().filter((o) => o.status === "Review").length,
  );
  ...
  readonly volumeTicks = computed<number[]>(() => {
    ...
    return [nice, nice * 0.75, nice * 0.5, nice * 0.25, 0].map((v) =>
      Math.round(v),
    );
  });
  ...
  readonly workflowData = computed<WorkflowDataPoint[]>(() => {
    ...
    return WORKFLOW_STAGES.map((stage) => ({
      name: stage.name,
      color: stage.color,
      count: this.orders().filter((o) => stage.statuses.includes(o.status))
        .length,
    }));
  });

  readonly revenue = computed(() =>
    this.formatUtils.formatCurrency(
  ...
  readonly turnaround = computed(() => {
    ...
    const avg =
      sent.reduce(
        (sum, o) =>
          sum +
          (Date.parse(o.sentAt as string) - Date.parse(o.receivedAt)) /
            86400000,
        0,
      ) / sent.length;
  ...
  readonly openChangeRequests = computed(() => {
    ...
    return all.filter((c) => c.status === "Pending" || c.status === "In Review")
      .length;
  });
  ...
  readonly quickStats = computed<QuickStatData[]>(() => [
    ...
    },
    {
      ...
      value: `${this.openChangeRequests()} open`,
      sub: "2 require action today",
      icon: "alert-triangle",
      color: "text-warning",
    },
  ]);
