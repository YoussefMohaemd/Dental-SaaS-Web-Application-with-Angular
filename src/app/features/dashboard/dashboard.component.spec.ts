...
describe('DashboardComponent', () => {
        ...
        },
        {
          ...
          useValue: {
            notifications: signal([
              { id: 'n1', type: 'order', title: 'New Order', message: 'Order received', read: false, createdAt: '2024-12-16T10:00:00Z' },
            ]),
            loading: signal(false),
  ...
  it('should display a New Order button that navigates to createOrder', () => {
    ...
    const newOrder = buttons.find((b) => (b.nativeElement.textContent ?? '').includes('New Order'));
  ...
  it('should render all action icons (no empty icon slots)', () => {
    for (const name of ['plus', 'chevron-right', 'chevron-right-lg', 'arrow-right', 'activity', 'alert-triangle-sm']) {
