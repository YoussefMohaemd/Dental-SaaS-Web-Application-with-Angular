...
Current automated baseline: **357 passing unit tests** (`npm test -- --watch=false --browsers=ChromeHeadless`).
...
| Empty | `select.component.spec.ts::renders the placeholder and options` | `table-feedback.component.spec.ts::should default to empty mode` | `orders.component.spec.ts::should show the empty state when no orders are available` | — | `order-data.service.spec.ts::(constructor load + empty flush in create-order test)` |
...
| Permission / security | — | — | — | `auth.guard.spec.ts::redirects to /login when auth marker is missing`; `auth.interceptor.spec.ts::attaches an Authorization header when a token exists` | `auth.interceptor.spec.ts::forwards original request when no token exists` |
